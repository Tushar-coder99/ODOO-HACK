...
const Profile = () => {
    ...
    useEffect(() => {
        if (!userInfo) {
            ...
            const fetchOrders = async () => {
                try {
                    const res = await fetch(`${API_URL}/orders/myorders/${userInfo._id}`); // USE IT HERE
    ...
    return (
        <div className="section" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            {/* ... keep your existing JSX code for displaying orders ... */}
            {/* If you need the full JSX again, let me know, but logic update is just the fetch line */}
                    ...
                    {orders.map((order) => (
                        <div key={order._id} style={{ border: '1px solid #e2e8f0', padding: '1rem', borderRadius: '8px', background: 'white' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', borderBottom: '1px solid #eee', paddingBottom: '0.5rem' }}>
                                ...
                                <span style={{ color: 'green', fontWeight: 'bold' }}>Paid: ${order.totalPrice.toFixed(2)}</span>
                            </div>
